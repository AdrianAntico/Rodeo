...
AutoWord2VecScoring <- function(data,
  ...
  if(tolower(BuildType) == "individual") {
    ...
    for(textCol in stringCol) {
      data1 <- Rodeo::AutoH2OTextPrepScoring(data = data, string = textCol, NThreads = Threads, MaxMem = MaxMemory, StartH2O = FALSE)
      Scores <- data.table::as.data.table(h2o::h2o.transform(model, words = data1, aggregate_method = "AVERAGE"))
      data.table::setnames(Scores, names(Scores), paste0(textCol, "_", names(Scores)))
      if(!KeepStringCol) {
        data[, eval(textCol) := NULL]
...
AutoEncoder_H2O <- function(RunMode = 'train',
  ...
  if(tolower(RunMode) == "train") {
    ...
    # Score model
    ScoringData. <- Rodeo::H2OAutoencoderScoring(
...
IsolationForest_H2O <- function(ArgsList=ArgsList,
  ...
  if(tolower(RunMode) == "train") {
    ...
    # Run function
    TrainData. <- Rodeo::H2OIsolationForest(
      data = TrainData.,
      Features = ColsUsed,
    ...
    if(!is.null(ValidationData.)) {
      ...
      # Score model
      ValidationData. <- Rodeo::H2OIsolationForestScoring(
        data = ValidationData.,
    ...
    if(!is.null(TestData.)) {
      ...
      # Score model
      TestData. <- Rodeo::H2OIsolationForestScoring(
        data = TestData.,
    ...
    # Score model
    ScoringData. <- Rodeo::H2OIsolationForestScoring(
      data = ScoringData.,
      Features = ArgsList$FE_H2OIsolationForestScoring$Features,
      IDcols = ArgsList$FE_H2OIsolationForestScoring$IDcols,
      H2OStart = TRUE,
...
Clustering_H2O <- function(ArgsList=ArgsList_FEE,
  ...
  if(tolower(RunMode) == "train") {
    ...
    # Run function
    TrainData. <- Rodeo::AutoClustering(
      data = TrainData.,
      Features = ColsUsed,
    ...
    if(!is.null(ValidationData.)) {
      ...
      # Score model
      ValidationData. <- Rodeo::AutoClusteringScoring(
        data = ValidationData.,
    ...
    if(!is.null(TestData.)) {
      ...
      # Score model
      TestData. <- Rodeo::AutoClusteringScoring(
        data = TestData.,
    ...
    # Score model
    ScoringData. <- Rodeo::AutoClusteringScoring(
      data = ScoringData.,
      FeatureColumns = ArgsList$FE_H2OClusteringScoring$FeatureColumns,
      ModelID = ArgsList$FE_H2OClusteringScoring$ModelID,
      SavePath = ArgsList$FE_H2OClusteringScoring$SavePath,
